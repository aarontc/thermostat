module Beagle
  ...
  class Gpio
    ...
    def initialize(pin, options = {})
	  ...
	  self.active_low = @options[:active_low] || :false
	...
	def active_low=(value)
		raise ArgumentError, 'Invalid active low value' unless [0, 1, false, true].include? value
		...
		File.open(File.join(PATHS[:gpio], "gpio#{@pin.to_s}", "active_low"), 'w') do |gpio_value|
          gpio_value << val
        end
	end
    ...
    def active_low?
      File.open(File.join(PATHS[:gpio], "gpio#{@pin.to_s}", "active_low")) do |gpio_value|
        @active_low = gpio_value.read
      end

      if @active_low.to_i == 1
        true
      elsif @active_low.to_i == 0
        false
      end
    end

    def needs_export?
      return false if Beagle::GPIOEXPORTS.keys.include?(@pin)
	  !Dir.exist? File.join(PATHS[:gpio], "gpio#{@pin.to_s}")
    end
